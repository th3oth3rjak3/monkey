...
func TestBooleanExpressions(t *testing.T) {
	tests := []vmTestCase{
		...
		{"!!false", false},
		{"!!5", true},
		{"!(if (false) { 5; })", true}, // should produce Null, which negated would be true
		{"if ((if (false) { 10 })) { 10 } else { 20 }", 20}, // The conditional will evaluate to Null, so it should be properly be converted to false
	}
...
func testExpectedObject(t *testing.T, expected any, actual object.Object) {
	...
	default:
		t.Errorf("unhandled expected type. got=%T", expected)
	}
}
